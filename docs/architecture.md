...
通用 `extract-book-principles` Skill 负责“怎样处理书”；`book_principles` Python 工具包负责可重复的解析、定位、校验和渲染；书籍知识包保存“处理后的内容”；独立书籍 Skill 负责“怎样把这本书用于具体任务”。

```text
通用提炼 Skill ──调用──> Python 工具包
      │                    │
      └────────生成────────┘
               ↓
标准化书籍知识包
...
工具包不负责判断一句话是否构成原则，也不替代语义审核。Skill 不重复实现格式解析器，而是调用工具包并根据解析结果完成提炼与审核。
